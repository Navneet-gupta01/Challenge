...
trait RepositoryImplicits {
  implicit def userRepositoryHandler[F[_] : Monad](implicit T: Transactor[F]): UserRepository.Handler[F] =
...
trait ErrorHandlerImplicit {
  ...
  implicit def userHttpErrorHandler: HttpErrorHandler[IO, UserDomainErrors] = new UserHttpErrorHandler[IO]
}